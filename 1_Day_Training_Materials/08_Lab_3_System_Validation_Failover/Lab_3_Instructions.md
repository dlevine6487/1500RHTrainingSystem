...
### Step 1: Pinch Point - H-CiR Limitations
1. Ensure the system is in **RUN-Redundant** mode.
2. In TIA Portal, attempt to add a new PROFINET device (e.g., a standard drive) to the Blue Ring in the Network View.
3. Click **Download to device** (Hardware configuration).
4. **Observation:** TIA Portal will block the download and throw a warning that the module must be stopped. Hardware Configuration in RUN (H-CiR) has limitations in 1500RH systems.
5. **Correction:** To perform this download, you must manually stop the Backup CPU, download the configuration, transition the Primary to STOP, and start the Backup CPU to make it the new Primary with the updated configuration. (For now, just delete the dummy device to proceed).
...
### Step 3: CPU Failover Simulation
...
### Step 4: Diagnostic Evaluation (OB72)
...
### Step 5: Network Break Simulation
...
### Step 6: Diagnostic Evaluation (OB70)
